from PySide6.QtCore import (QDateTime, QThread, Signal, QTime, QTimer, Qt)
    ...
    def run(self):
        try:
            if self.method == "nmcli":
                ...
                # Gespeicherte Verbindung wieder aktivieren
                if self.ssid:
                    # Verbindungsprofil per Name (id) hochfahren
                    subprocess.run(
                        ["nmcli", "connection", "up", "id", self.ssid],
                        check=False
                    )

            elif self.method == "wpa":   # Raspberry Pi OS Lite / wpa_supplicant
...
class PrayerTimeClockWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.current_prayers = [
            self.current_day_fajr_time,
            self.current_day_shroq_time,
            self.current_day_zohr_time,
            self.current_day_asr_time,
            self.current_day_magrb_time,
            self.current_day_isha_time
        ]

        self.current_prayers_labels = [
            self.current_day_fajr,
            self.current_day_shroq,
            self.current_day_zohr,
            self.current_day_asr,
            self.current_day_magrb,
            self.current_day_isha
        ]

        self.tomorrows_prayers = [
            self.next_day_fajr_time,
            self.next_day_shroq_time,
            self.next_day_zohr_time,
            self.next_day_asr_time,
            self.next_day_magrb_time,
        ...
        self.next_day_prayer_boxes = [
            self.next_day_fajr_box,
            self.next_day_shroq_box,
            self.next_day_zohr_box,
            self.next_day_asr_box,
            self.next_day_magrb_box,
            self.next_day_isha_box
        ]
        ...
        # Name der aktiven WiFi-Verbindung ermitteln (für nmcli-Reconnect)
        self.wifi_connection_name = self._get_active_wifi_connection_name()
        print("Aktive WiFi-Connection:", self.wifi_connection_name)
    ...
    def _get_active_wifi_connection_name() -> str | None:
        """
        Liest den Namen der aktiven WiFi-Connection aus nmcli aus.
        Gibt z.B. 'HOME-WIFI' zurück oder None, wenn nichts aktiv ist.
        """
        try:
            ...
            print("Fehler beim Lesen der aktiven WiFi-Verbindung:", e)
        return None
    ...
    def _try_reconnect_and_refresh(self):
        ...
        self.reconnect_worker = ReconnectWorker(
            method="nmcli",
            ssid=self.wifi_connection_name,  # hier: aktive WiFi-Connection übergeben
            iface="wlan0",
            parent=self
        )
    ...
    def __setupData(self):
        ...
        if prayer_times["requestSuccess"][0] is True:
    ...
    def update_timer(self):
        ...
        if self.current_prayer_time is None:
            ...
            self.current_prayer_time = QDateTime.fromString(
                today_str + " " + next_prayer_time_str, "yyyy-MM-dd hh:mm"
            )
        else:
            if self.current_prayer_index == 5 or self.current_prayer_index == 0:
                print(self.current_prayer_index)
                self.current_prayer_time = QDateTime.fromString(
                    today_str + " " + self.prayer_times["Prayers"][self.current_prayer_index],
                    "yyyy-MM-dd hh:mm"
                )
            else:
                print(self.current_prayer_index)
                self.current_prayer_time = QDateTime.fromString(
                    today_str + " " + self.prayer_times["Prayers"][self.current_prayer_index + 1],
                    "yyyy-MM-dd hh:mm"
                )
